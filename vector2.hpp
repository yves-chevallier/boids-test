...
class Vector2 {
   public:
    ...
    auto angle(const class_type& right) const {
        return atan2(right.y - y, right.x - x);
    }
    auto angle() const { return atan2(y, x); }

    auto toroidal_distance(const class_type &other, value_type width, value_type height) const
    {
        return sqrt(toroidal_distance2(other, width, height));
    }

    auto toroidal_distance2(const class_type &other, value_type width, value_type height) const
    {
